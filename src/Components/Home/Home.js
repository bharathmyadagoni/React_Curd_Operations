...
useEffect(() => {
    ...
    },[starData]); //Store the StarData in localStorage
useEffect(() => {
...
const titleHandler = (event) => {
  ...
  const LengthofData = (data.length)+deleteData.length; //length of the All data
...
const add = () => {
 ...
 const cancelHandler = () => {
  ...
  const deleteSeachData = JSON.parse(localStorage.getItem("deletedData"))>1?JSON.parse(localStorage.getItem("deletedData")):searchDeletedata
  // Search Star Notes Funtion
return (
    <div>
      {showAdd ? (
          ...
          <div className="container-2">
            ...
            {activeTab === 0 && (
              <div>
                ...
                <div>
                  {JSON.parse(localStorage.getItem("deletedData")).map((each, index) => {
                    return (
